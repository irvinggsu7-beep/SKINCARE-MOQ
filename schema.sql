...
CREATE TABLE IF NOT EXISTS public.citas (
    ...
    hora_inicio TIME NOT NULL,
    hora_fin TIME NOT NULL,
    limite_pago TIMESTAMP WITH TIME ZONE NOT NULL,
    tipo_tolerancia VARCHAR(20) DEFAULT 'regular',
    estado VARCHAR(30) DEFAULT 'pendiente' CHECK (estado IN ('pendiente', 'confirmada', 'completada', 'no_acudio', 'cancelada')),
    metodo_liquidacion VARCHAR(30) DEFAULT 'Electronico', -- 'Electronico' o 'Efectivo'
    notas TEXT,
    creado_en TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ====================================================================
-- HABILITAR ROW LEVEL SECURITY (RLS) Y POLÍTICAS DE ACCESO SEGURA
-- ====================================================================
...
DROP POLICY IF EXISTS "Permitir lectura publica de cupones" ON public.cupones;
CREATE POLICY "Permitir lectura publica de cupones" ON public.cupones FOR SELECT USING (true);

DROP POLICY IF EXISTS "Permitir lectura publica de bloqueos" ON public.bloqueos_agenda;
CREATE POLICY "Permitir lectura publica de bloqueos" ON public.bloqueos_agenda FOR SELECT USING (true);

DROP POLICY IF EXISTS "Permitir lectura publica de citas" ON public.citas;
...
DROP POLICY IF EXISTS "Permitir crear citas anonimo" ON public.citas;
CREATE POLICY "Permitir crear citas anonimo" ON public.citas FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Permitir actualizar citas anonimo" ON public.citas;
CREATE POLICY "Permitir actualizar citas anonimo" ON public.citas FOR UPDATE USING (true);

DROP POLICY IF EXISTS "Permitir administrar servicios" ON public.servicios;
CREATE POLICY "Permitir administrar servicios" ON public.servicios FOR ALL USING (true);

DROP POLICY IF EXISTS "Permitir administrar cupones" ON public.cupones;
CREATE POLICY "Permitir administrar cupones" ON public.cupones FOR ALL USING (true);

DROP POLICY IF EXISTS "Permitir administrar bloqueos" ON public.bloqueos_agenda;
CREATE POLICY "Permitir administrar bloqueos" ON public.bloqueos_agenda FOR ALL USING (true);

DROP POLICY IF EXISTS "Permitir administrar configuracion" ON public.configuracion;
